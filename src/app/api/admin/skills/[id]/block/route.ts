...
import { sendBlockEmail } from '@/lib/skill-management-emails';

type RouteContext = { params: Promise<{ id: string }> };

/**
 * POST /api/admin/skills/[id]/block
 * Blocage definitif par l'admin.
 */
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    if (skill.status === 'blocked') {
      return NextResponse.json({ error: 'Ce skill est deja bloque' }, { status: 400 });
    }
    ...
    const { error: updateError } = await serviceClient
      ...
      .update({
        ...
        blocked_permanently: permanent === true,
        is_visible: false,
        updated_at: now,
      })
      .eq('id', id);
    ...
    return NextResponse.json({ success: true, message: 'Skill bloque' });
  } catch (error) {
